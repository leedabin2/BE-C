...
> **개선 여지**: 현재 외부 서버의 5xx 오류를 재시도 대상(`CHANNEL_UNAVAILABLE`)으로 처리하고 있는데, 생각해보면 외부 서버가 이미 장애 상태일 때 계속 요청을 보내면 오히려 상황을 악화시킬 수 있다고 생각합니다. 실제로는 5xx가 일정 횟수 이상 반복되면 잠시 요청을 멈추는 서킷 브레이커(Circuit Breaker) 패턴을 함께 적용하는 것이 더 안전할 것 같습니다.
...
현재 FAILED 상태가 되면 스케줄러가 다시 픽업하지 않습니다. 실제 운영 환경이라면 아래와 같은 처리가 필요할 것 같다고 생각합니다.

1. **모니터링 연동**: FAILED 알림이 생겼을 때 운영팀이 바로 알 수 있도록 슬랙이나 이메일로 알림을 보내면 좋을 것 같습니다.
2. **수동 재시도 API**: 운영자가 FAILED 알림을 직접 확인하고 PENDING으로 되돌릴 수 있는 어드민 API가 있으면 유용할 것 같습니다.
3. **재시도 횟수 초기화 기준**:
   - 외부 채널 자체 장애가 복구된 경우 → `retry_count` 초기화 후 재시도 (장애 원인이 사라졌으므로)
   - 수신자 주소 오류 등 데이터 문제인 경우 → 초기화 없이 FAILED 유지 (같은 오류가 반복될 가능성이 높으므로)
...
- AI가 제안한 코드는 로컬에서 직접 실행해 실제로 동작하는지 확인했습니다.
- 엣지 케이스 분석 결과는 테스트 코드로 재현하거나 코드를 직접 읽어보며 사실 여부를 확인했습니다.
