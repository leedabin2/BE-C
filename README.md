...
결제 서버와 알림 서버가 분리된 구조에서 결제 서버가 알림 서버에 이벤트를 전달하다가 서버 종료나 네트워크 단절이 발생하면 메시지가 유실되고, 이를 단순히 `try-catch`로 무시하면 비즈니스 로직에 영향이 없는 것처럼 보이지만 실제로는 알림 발송 기록도 없고 재시도도 되지 않습니다. 이 문제를 해결하기 위해 **Transactional Outbox Pattern**을 적용했습니다.
